...
def _log(msg: Any, head: str, color: str, end: str) -> None:
    head_len = len(head) + 4
    space = " " * head_len
    lines = str(msg).split("\n")
    pfx = f"[{head}]: " if head else ""
    print(f"{color}{pfx}{lines[0]}" + "".join(f"\n{space}{l}" for l in lines[1:]) + Fore.RESET, end=end)


def print_warn(msg: str, header: str = "WARN", end: str = "\n"): 
    _log(msg, header, Fore.YELLOW, end)


def print_info(msg: str, header: str = "INFO", end: str = "\n"): 
    _log(msg, header, Fore.BLUE, end)


def print_good(msg: str, header: str = "OK", end: str = "\n"): 
    _log(msg, header, Fore.GREEN, end)


def print_error(msg: str, header: str = "ERR", end: str = "\n"): 
    _log(msg, header, Fore.RED, end)
...
def get_categorical_indices() -> list[int]:
    """
    Returns indices of categorical features strictly as defined in config.yaml.
    Assumes X columns follow the order of keys in cfg.dataset['features'].
    """
    from src.config import cfg
    features = cfg.dataset["features"]
    return [i for i, (name, dtype) in enumerate(features.items()) if dtype == "cat"]
